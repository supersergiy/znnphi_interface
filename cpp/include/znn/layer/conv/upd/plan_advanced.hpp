...
private:
    ...
    static const long_t KERNEL_NEXT_TYPE   = KERNEL_NEXT_OUTPUT * OFM_SETS;

    static const long_t INPUT_NEXT_BATCH =
        IFM_SETS * SIMD_WIDTH * IFD * IFH * IFD;
    static const long_t OUTPUT_NEXT_BATCH =
        OFM_SETS * SIMD_WIDTH * OFD * OFH * OFD;
    ...
    //                             t * k_per_thread,
    //                             std::min( (t+1) * k_per_thread,
    //                             total_kernel_elements) );
    //         };
    //         // std::cout << "Thread[ " << t << " ] FROM: " << t *
    //         k_per_thread
    //         //           << " TO " << std::min( (t+1) * k_per_thread,
    //         total_kernel_elements)
    //         //           << "\n";
    //     }
    // }
...
private:
    ...
    using problem =
        upd_problem_t<NUM_TYPES, upd_problem_size_t<BS, OFD, OFH>,
                      upd_ioshape_t<IFH * IFW * SIMD_WIDTH * IFM_SETS,
                                    IFH * IFW * SIMD_WIDTH, IFW * SIMD_WIDTH>,
                      upd_ioshape_t<OFH * OFW * SIMD_WIDTH * OFM_SETS,
                                    OFH * OFW * SIMD_WIDTH, OFW * SIMD_WIDTH>>;

    using problems_t = upd_split_problem_t<problem>;
...
private:
    ...
    void schedule_batch_group(long_t tno, float const* i, float const* o,
                              float* w)
    {
        using type = typename std::tuple_element<TYPE_NO, problems_t>::type;
        ...
        for (long_t x = 0; x < IO_PAIR_GROUPS; ++x)
        {
            ...
            //           << " to " << ( x * PAIRS_PER_GROUP + PAIRS_PER_GROUP -
            //           1 )
            //           << " on thread " << tno + x
            //           << "\n";

            // std::cout << "\tWork: " << type::size::depth
            // << ' ' << type::size::height
            // << ' ' << OFW << "\n";

            conv_fns[tno * THREADS_PER_TYPE + x] = [this, i, o, w, x]() {
                    ...
                    for (long_t io = 0; io < PAIRS_PER_GROUP; ++io)
                    {
                        work_type::execute(
                            i + this->pairs[x * PAIRS_PER_GROUP + io].ioffset +
                                type::ioffset + b * INPUT_NEXT_BATCH,
                            o + this->pairs[x * PAIRS_PER_GROUP + io].ooffset +
                                type::ooffset + b * OUTPUT_NEXT_BATCH,
    ...
    schedule_type(long_t tno, float const* i, float const* o, float* w)
    {
        ...
        // upd_problem_printer<typename
        // std::tuple_element<TYPE_NO,problems_t>::type>::print();

        schedule_batch_group<TYPE_NO>(tno, i, o, w);

        schedule_type<TYPE_NO + 1>(tno + 1, i, o, w + KERNEL_NEXT_TYPE);
    }

    void sch(float const* i, float const* o, float* w)
    {
...
public:
        ...
        , pairs(pair_split(IFM_SETS, OFM_SETS, 0, 0, 0))
    {
        ...
        // for ( const auto & e: xx )
        // {
        //     std::cout << "E: " << e.ioffset << ' ' << e.ooffset << ' ' <<
        //     e.koffset << "\n";
        // }
