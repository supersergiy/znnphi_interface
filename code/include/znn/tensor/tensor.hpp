...
private:
    ...
    void do_one_init( detail::tensor::host_tag )
    {
        std::fill_n(this->data(), this->num_elements(), static_cast<T>(1));
    }

    void do_one_init( detail::tensor::hbw_tag )
    {
        std::fill_n(this->data(), this->num_elements(), static_cast<T>(1));
    }

    void do_one_init( detail::tensor::device_tag )
    {
        T* rnd = reinterpret_cast<T*>(
            detail::tensor::malloc(this->num_elements()*sizeof(T),
                                   detail::tensor::host_tag()));
        std::fill_n(rnd, this->num_elements(), static_cast<T>(1));
        this->load(rnd, detail::tensor::host_tag());
        detail::tensor::free(rnd, detail::tensor::host_tag());
    }
