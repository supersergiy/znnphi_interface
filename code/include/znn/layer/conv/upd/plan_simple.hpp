...
template <long_t Threads, long_t BS, long_t OFM, long_t IFM, long_t IFD,
...
private:
    ...
    void collect_kernel(float const* __restrict w, float* __restrict k,
                        float rate, long_t from, long_t to)
    {
    ...
    void schedule_collect(float const* __restrict w, float* __restrict k,
                          float rate)
    {
...
private:
            ...
            for (long_t o = 0; o < ofm; ++o)
            {
                for (long_t i = 0; i < ifm; ++i)
                {
                    ret.push_back(upd_problem_args(
                        ...
                        koff + i * W_IN_STRIDE + i * W_OUT_STRIDE));
                }
            }
    ...
    using work_type =
        upd_work<dimension<OFD, IFH * IFW, OFH * OFW>, dimension<OFH, IFW, OFW>,
                 dimension<OFW, 1, 1>, conv_traits<CD, 1, 1>,
                 conv_traits<CH, 1, 1>, conv_traits<CW, 1, 1>>;

private:
    void schedule_threads(float const* i, float const* o, float* w)
    {
        ...
        for (long_t tno = 0; tno < Threads; ++tno)
        {
            std::cout << "\tScheduling thread " << tno
                      << " from: " << (tno * per_thread) << "  to "
                      << std::min(tno * per_thread + per_thread,
            ...
            conv_fns[tno] = [this, i, o, w, per_thread, tno]() {
                for (long_t b = 0; b < BS; ++b)
                {
                    ...
                    {
                        work_type::execute(
                            i + this->pairs[io].ioffset + b * INPUT_NEXT_BATCH,
                            o + this->pairs[io].ooffset + b * OUTPUT_NEXT_BATCH,
...
public:
    upd_plan(kernel_launcher* l, float const* i, float const* o, float* k,
             float* k_flip, float* b, float* w)
        : launcher(l)
        , conv_fns(Threads)
        , reduce_fns(Threads)
