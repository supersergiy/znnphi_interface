...
{
	tensors["user_output"] = new znn::phi::hbw_array<float>(72);
	tensors["input"] = new znn::phi::hbw_array<float>(32);
	tensors["user_input"] = new znn::phi::hbw_array<float>(32);
	tensors["input_padded"] = new znn::phi::hbw_array<float>(128);
	tensors["output"] = new znn::phi::hbw_array<float>(72);
	
	layers["conv_0_padder"] = new znn::phi::PadLayer(1, 1, 1, 2, 0, 1);
	layers["conv_0"] = new znn::phi::ConvWrapper(1, 1, 1, 1, 4, 1, 2, 0, 0, false, 2, 2);
	...
	layers["unblock_output"] = new znn::phi::UnblockDataLayer(1, 1, 1, 3);
	layers["block_input"] = new znn::phi::BlockDataLayer(1, 1, 1, 2);
	
	input_size = 4;
	out_dim = 5;
	size_t tmp_shape[] = { 1, 1, 1, 3, 3 };
	out_shape.assign(tmp_shape, tmp_shape + 5);
	size_t tmp_strides[] = { 36, 36, 36, 12, 4 };
...
void znn::phi::Znet::forward(void)
{
	...
	for (int i = 0; i < 1; i++) {
		...
		auto begin = std::chrono::high_resolution_clock::now();
		for (int i = 0; i < 1; i++) {
			layers["conv_0_padder"]->forward(tensors["input"]->data(), tensors["input_padded"]->data(), NULL, NULL);
		}
		auto end = std::chrono::high_resolution_clock::now();
		auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count();
		double secs = static_cast<double>(duration) / 1000000;
		std::cout << "conv_0_padder: " << secs/1 << "\n";
		}
		{
		auto begin = std::chrono::high_resolution_clock::now();
		for (int i = 0; i < 1; i++) {
		...
		auto begin = std::chrono::high_resolution_clock::now();
		for (int i = 0; i < 1; i++) {
			layers["conv_0"]->forward(tensors["input_padded"]->data(), tensors["output"]->data(), tensors["conv_0_kernel"]->data(), tensors["conv_0_bias"]->data());
		}
		auto end = std::chrono::high_resolution_clock::now();
		auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count();
		double secs = static_cast<double>(duration) / 1000000;
