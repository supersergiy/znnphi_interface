...
public:
   ...
   {
      typedef const float (*k_tp)[ifm/S][kd][khw][khw][S][S];
      typedef float (*subk_tp)[ifm/S][kd/stride_d][khw/stride_hw][khw/stride_hw][S][S];
      ...
      for (int d = start_d; d < kd; d += stride_d, ++sub_d) {
         for (int h = start_h; h < khw; h += stride_hw, ++sub_h) {
            for (int w = start_w; w < khw; w += stride_hw, ++sub_w) {
               for (int i = 0; i < ifm/S; ++i) {
                  for (int o = 0; o < ofm/S; ++o) {
   ...
   std::string getParamString(int kd_start, int kw_start, int kh_start)
   {
      std::stringstream ss; 
      ...
      ss << "OUT_PADD_FRONT="  << out_padd + kd_start << " ";
      ss << "OUT_PADD_BACK="   << out_padd << " ";

      ss << "OUT_PADH_FRONT="  << out_padhw + kh_start << " ";
      ss << "OUT_PADW_FRONT="  << out_padhw + kw_start << " ";
      ss << "OUT_PADHW_BACK="  << out_padhw << " ";
   ...
   {   
      ...
      int subk_size = ifm * ofm * kd * khw * khw / (stride_d * stride_hw * stride_hw);
