...
def set_mergecrop_dim(params, bot_tensors):
    ...
    params["id1"]  = bot_tensors[0].dim[2]
    params["ihw1"] = bot_tensors[0].dim[3]
    params["ifm"]  = bot_tensors[0].dim[1]
    params["id"]   = bot_tensors[0].dim[2]
    params["ihw"]  = bot_tensors[0].dim[3]
    params["ifm2"] = bot_tensors[1].dim[1]
    params["id2"]  = bot_tensors[1].dim[2]
    params["ihw2"] = bot_tensors[1].dim[3]

    params["crop_top_dim"] = top_dim
    top_dim[1] = params["ifm1"]
