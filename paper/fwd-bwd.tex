\section{Forward and backward propagation}
  ...
  Note how the loop over $f$ comes before the loop over $k_z$.  We
  expect the compiler to unroll the loop over $k_z$.  When auxiliary
  registers are required, the compiler is expected to reorder the load
  and multiply--add operations such that the values loaded in the
  ...
  Therefore we choose $R = 10$ registers for
  ...
  (one register is devoted to $wreg$). Furthermore, $R$ is greater than $nl$, as $nl \le 8$ for all modern CPUs.
