...
znn::phi::Znet::Znet(std::string weights_path)
{
	...
	layers["Deconvolution2"] = new znn::phi::DeconvAsConvLayer(1, 80, 64, 18, 12, 1, 2, 1, 2, 0, 0, false, false, tensors["Deconvolution2_kernel"]->data());
	...
	layers["Deconvolution4"] = new znn::phi::DeconvAsConvLayer(1, 48, 36, 18, 48, 1, 2, 1, 2, 0, 0, false, false, tensors["Deconvolution4_kernel"]->data());
	...
	layers["Deconvolution3"] = new znn::phi::DeconvAsConvLayer(1, 64, 48, 18, 24, 1, 2, 1, 2, 0, 0, false, false, tensors["Deconvolution3_kernel"]->data());
	...
	layers["deconv_d3"] = new znn::phi::DeconvAsConvLayer(1, 36, 28, 18, 96, 1, 2, 1, 2, 0, 0, false, false, tensors["deconv_d3_kernel"]->data());
...
void znn::phi::Znet::forward(void)
{
	...
	for (int i = 0; i < 1; i++) {
		...
		auto begin = std::chrono::high_resolution_clock::now();
		for (int i = 0; i < 1; i++) {
			layers["Deconvolution1"]->forward(tensors["conv0_d5"]->data(), tensors["Deconvolution1"]->data(), tensors["Deconvolution1_kernel"]->data(), tensors["Deconvolution1_bias"]->data(), NULL );
		}
		auto end = std::chrono::high_resolution_clock::now();
		auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count();
		double secs = static_cast<double>(duration) / 1000000;
		...
		auto begin = std::chrono::high_resolution_clock::now();
		for (int i = 0; i < 1; i++) {
			layers["Deconvolution2"]->forward(tensors["conv4_d4"]->data(), tensors["Deconvolution2"]->data(), tensors["Deconvolution2_kernel"]->data(), tensors["Deconvolution2_bias"]->data(), NULL );
		}
		auto end = std::chrono::high_resolution_clock::now();
		auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count();
		double secs = static_cast<double>(duration) / 1000000;
		...
		auto begin = std::chrono::high_resolution_clock::now();
		for (int i = 0; i < 1; i++) {
			layers["Deconvolution3"]->forward(tensors["conv4_d3"]->data(), tensors["Deconvolution3"]->data(), tensors["Deconvolution3_kernel"]->data(), tensors["Deconvolution3_bias"]->data(), NULL );
		}
		auto end = std::chrono::high_resolution_clock::now();
		auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count();
		double secs = static_cast<double>(duration) / 1000000;
		...
		auto begin = std::chrono::high_resolution_clock::now();
		for (int i = 0; i < 1; i++) {
			layers["Deconvolution4"]->forward(tensors["conv4_d2"]->data(), tensors["Deconvolution4"]->data(), tensors["Deconvolution4_kernel"]->data(), tensors["Deconvolution4_bias"]->data(), NULL );
		}
		auto end = std::chrono::high_resolution_clock::now();
		auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count();
		double secs = static_cast<double>(duration) / 1000000;
		...
		auto begin = std::chrono::high_resolution_clock::now();
		for (int i = 0; i < 1; i++) {
			layers["deconv_d3"]->forward(tensors["conv4_d1"]->data(), tensors["deconv_d3"]->data(), tensors["deconv_d3_kernel"]->data(), tensors["deconv_d3_bias"]->data(), NULL );
		}
		auto end = std::chrono::high_resolution_clock::now();
		auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count();
		double secs = static_cast<double>(duration) / 1000000;
