\section{Update phase}
  ...
  each thread to perform an independent computation.  In the update
  phase, the computed $\Delta W$ tensor

  $\hat{I}$ of size $B \times \alpha S \times N_x \times N_y \times
  N_z$, $\hat{I}'$ of size $B \times \alpha' S \times N_x \times N_y
  \times N_z$ to produce $\Delta W$ of size $\alpha' S \times \alpha S
  \times K_x \times K_y \times K_z$


  \quad The main goal of the primitive is
