\section{Forward and backward propagation}
  ...
  \begin{algorithm}
    {\footnotesize
      \begin{codebox}
        \Procname{$\proc{Fwd-Bwd-Subtask} \langle \Theta, R_z, R_y, R_x, K_z, K_y, K_x \rangle(i,w,o,\theta)$}
        \li \kw{simd float} $oreg[R_z][R_y][R_x]$
        ...
        \End \li \kw{end if}
        \li \kw{for} $k_z \gets 0 \To K_z-1$
        \li   \Do \kw{for} $k_y \gets 0 \To K_y-1$
        \li      \Do \kw{for} $k_x \gets 0 \To K_x-1$
        \li         \Do \kw{for} $s \gets 0 \To S - 1$ \Comment Partially unrolled
        \li         \Do $wreg \gets w[k_z][k_y][k_x][s][:]$
        \li \kw{for} $z \gets 0 \To R_z-1$ \Comment Fully unrolled
        \li   \Do \kw{for} $y \gets 0 \To R_y-1$ \Comment Fully unrolled
        \li      \Do \kw{for} $x \gets 0 \To R_x-1$ \Comment Fully unrolled
