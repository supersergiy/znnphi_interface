...
private:
    ...
    using problem = upd_problem_t< NUM_TYPES,
                                   upd_problem_size_t< BS, OFD, OFH >,
                                   upd_ioshape_t<
                                       IFH * IFW * SIMD_WIDTH * IFM_SETS,
                                              IFH * IFW * SIMD_WIDTH,
                                       IFW * SIMD_WIDTH >,
                                   upd_ioshape_t<
                                       OFH * OFW * SIMD_WIDTH * OFM_SETS,
                                       OFH * OFW * SIMD_WIDTH,
                                       OFW * SIMD_WIDTH >>;
...
private:
    ...
    void schedule_batch_group( long_t tno,
                               float const * i,
                               float const * o,
                               float *       w )
    {
        ...
        for ( long_t x = 0; x < IO_PAIR_GROUPS; ++x )
        {
            ...
            conv_fns[tno * THREADS_PER_TYPE + x] = [this,i,o,w,x]() {
                ...
                dimension<type::size::depth,IFH*IFW,OFH*OFW>,
                ...
                for ( long_t b = 0; b < type::size::batch; ++b )
                {
                    ...
                    {
                        work_type::execute( i
                                            + this->pairs[x * PAIRS_PER_GROUP + io].ioffset + type::ioffset
                                            + b * INPUT_NEXT_BATCH,
                                            o + this->pairs[x * PAIRS_PER_GROUP + io].ooffset + type::ooffset
                                            + b * OUTPUT_NEXT_BATCH,
