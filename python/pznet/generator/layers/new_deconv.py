...
def allocate_deconv_lines(lparam):
    if "group" in lparam and lparam["group"] == lparam["ifm"] and lparam["ifm"] == lparam["ofm"]:
        return allocate_interpolation(lparam)
    elif lparam["stride"][0] == lparam["kernel_dim"][2] and lparam["stride"][1] == lparam["kernel_dim"][3]: 
        import pdb; pdb.set_trace()
        return allocate_deconv_as_conv_lines(lparam)

def allocate_deconv_as_conv_lines(lparam):
    l = lparam
