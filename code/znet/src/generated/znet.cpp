...
znn::phi::Znet::Znet(std::string weights_path)
{
	...
	layers["Deconvolution1"] = new znn::phi::DeconvAsConvLayer(1, 96, 80, 18, 6, 1, 2, 1, 2, 0, 0, false, true, tensors["Deconvolution1_kernel"]->data());
	tensors["Deconvolution1_scale"] = new znn::phi::hbw_array<float>(80);
	readArrayFromFile(tensors["Deconvolution1_scale"]->data(), weights_path + "Deconvolution1_scale.data");
...
void znn::phi::Znet::forward(void)
{
	...
	for (int i = 0; i < 1; i++) {
		...
		auto begin = std::chrono::high_resolution_clock::now();
		for (int i = 0; i < 1; i++) {
			layers["Deconvolution1"]->forward(tensors["conv0_d5"]->data(), tensors["sum0_d4"]->data(), tensors["Deconvolution1_kernel"]->data(), tensors["Deconvolution1_bias"]->data(), tensors["Deconvolution1_scale"]->data());
		}
		auto end = std::chrono::high_resolution_clock::now();
		auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count();
		double secs = static_cast<double>(duration) / 1000000;
		...
		auto begin = std::chrono::high_resolution_clock::now();
		for (int i = 0; i < 1; i++) {
			layers["BatchNorm4"]->forward(tensors["conv0_d4"]->data(), tensors["Eltwise1"]->data(), tensors["scale_BatchNorm4"]->data(), tensors["bias_BatchNorm4"]->data());
		}
		auto end = std::chrono::high_resolution_clock::now();
		auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count();
		double secs = static_cast<double>(duration) / 1000000;
