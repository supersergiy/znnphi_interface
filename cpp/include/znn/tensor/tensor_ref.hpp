...
class const_tensor_ref : public detail::tensor::value_accessor<T, D, A>
{
private:
...
protected:
    const_tensor_ref() noexcept : extents_(0), strides_(0), ptr_(nullptr) {}
...
public:
    ...
    {
        return super_type::access(znn::phi::type_t<const_reference>(), i, ptr_,
                                  extents_.data(), strides_.data());
    }

    zi::vl::vec<long_t, D> const& shape_vec() const { return extents_; }

    long_t const* shape() const { return extents_.data(); }

    long_t num_elements() const { return strides_[0]; }

    TPtr data() const { return ptr_; }
    ...
    {
        detail::tensor::copy_n(ptr_, strides_[0], d, architecture(), tag);
    }
    ...
    typename std::enable_if<std::is_same<X, detail::tensor::hbw_tag>::value,
                            hbw_ptr<T const>>::type
    ptr() const
    {
        return hbw_ptr<T const>(ptr_);
    }
    ...
    template <typename X = A>
    typename std::enable_if<std::is_same<X, detail::tensor::host_tag>::value ||
                                std::is_same<X, detail::tensor::hbw_tag>::value,
                            TPtr>::type
    begin() const
    {
        return ptr_;
    ...
    zi::vl::vec<long_t, D> const& size() const { return extents_; }
};

template <class T, size_t D, class A>
class tensor_ref : public const_tensor_ref<T, D, A, T*>
{
private:
...
public:
    ...
    tensor_ref& operator=(tensor_ref const& other) noexcept
    {
        if (this != &other)
        {
            ZI_ASSERT(std::equal(this->shape(), this->shape() + dimensionality,
                                 other.shape()));
    ...
    {
        return super_type::access(znn::phi::type_t<reference>(), i,
                                  super_type::ptr_, super_type::extents_.data(),
    ...
    T const* data() const { return this->ptr_; }
    ...
    {
        detail::tensor::copy_n(d, this->strides_[0], this->ptr_, tag,
                               architecture());
    }
    ...
    ptr() const
    {
        return hbw_ptr<T const>(this->ptr_);
    }
...
template <class T, size_t D>
using hbw_tensor_ref = tensor_ref<T, D, detail::tensor::hbw_tag>;

template <class T, size_t D>
using device_tensor_ref = tensor_ref<T, D, detail::tensor::device_tag>;

template <class T, size_t D>
using host_const_tensor_ref = const_tensor_ref<T, D, detail::tensor::host_tag>;

template <class T, size_t D>
using hbw_const_tensor_ref = const_tensor_ref<T, D, detail::tensor::hbw_tag>;
...
template <class T>
using hbw_array_ref = tensor_ref<T, 1, detail::tensor::hbw_tag>;

template <class T>
using device_array_ref = tensor_ref<T, 1, detail::tensor::device_tag>;

template <class T>
using host_const_array_ref = const_tensor_ref<T, 1, detail::tensor::host_tag>;

template <class T>
using hbw_const_array_ref = const_tensor_ref<T, 1, detail::tensor::hbw_tag>;
