...
private:
    ...
    {
        return ( w == 1 ) ? w :
            (( (z+w-2)/(w-1) == x ) ? pick_best( z, x, w-1 ) : w);
    }

public:
    static const long_t w = pick_best( W, (W+SIMD_W_BLOCK-1)/SIMD_W_BLOCK, SIMD_W_BLOCK );
    static const long_t width = w;
private:
    static const long_t rem_for_h = SIMD_MAX_BLOCK / w;
public:
    static const long_t h = 1; //pick_best( H, (H+rem_for_h-1)/rem_for_h, rem_for_h );
    static const long_t height = h;
private:
    static const long_t rem_for_d = SIMD_MAX_BLOCK / w / h;
public:
    static const long_t d = 1; //pick_best( D, (D+rem_for_d-1)/rem_for_d, rem_for_d );
